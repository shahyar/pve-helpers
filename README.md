...
This applies the given list of host cores to the whole guest parent process.
Then, it assigns each guest CPU core to the specific host core from the list.

For best performance, save at least 1 core (eg. 0) for the host machine, and
match cores by cache topology. See `lscpu -e` or `lstopo`.

For example for `i7-8700` each core has two threads: 0-6, 1-7, 2-8, etc., and each
of those pairs share the same cache topology.
...
`#cpu_taskset 2-3,8-9` and `cores: 4`, you can create a 2-core, 4-thread guest,
with the following deterministic topology being replicated:
```text
Guest CPU 0 = Host CPU 2
Guest CPU 1 = Host CPU 3
Guest CPU 2 = Host CPU 8 (virtual)
Guest CPU 3 = Host CPU 9 (virtual)
```
...
### 2.3. `set_halt_poll` and `set_guest_halt_poll`

`set_halt_poll` changes the value of the kvm parameter `halt_poll_ns` in `/sys/module/kvm/parameters/halt_poll_ns`,
while `set_guest_halt_poll` changes `/sys/module/haltpoll/parameters/guest_halt_poll_ns`.
...
This functions similarly to `cpu_taskset`, in that each IRQ is assigned to one of the CPU cores from the list. However, it is less deterministic, as the order of the IRQs is not necessarily the same at every boot. This also means that for any `hostpciN` devices with multiple IRQs are assigned each possibly to a different core.
